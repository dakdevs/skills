...
Derive everything exclusively from the current `git diff main...HEAD` and fresh
CallDiff output. Every section (Why, Summary, What, Who this helps, Call diff,
Diagram) must be written from scratch based solely on the branch as it exists right
now.**
...
npx calldiff@latest main HEAD          # TypeScript call-stack diff for PR body
```
...
<verbatim plain-text output from `npx calldiff@latest main HEAD` in a fenced
`text` block>
...
**Never** include any AI attribution anywhere - no "Generated with Claude Code", no "Generated by Claude", no "Co-authored by AI", no bot footers, nothing. Not in the PR title, body, or commit messages created in step 8.
...
### 6. Add the CallDiff section

Use the CallDiff stdout gathered in step 1. The command must compare the same
committed branch range used for the rest of the PR:

```bash
npx calldiff@latest main HEAD
```

Do not run bare `npx calldiff@latest` here. With no refs, CallDiff compares `HEAD`
to the working tree, which is empty after step 0 commits the changes.

Add the command's stdout to the main PR description immediately after `## What`:

````markdown
## Call diff

```text
<verbatim CallDiff stdout>
```
````

Rules:

- Always include this section, even when CallDiff reports no call-stack changes.
- Capture stdout only. Do not include npm installation warnings or other stderr.
- Preserve the call tree and its `+` / `-` markers verbatim inside the `text` fence.
- If CallDiff reports no changes, write `No TypeScript call-stack changes
  detected.` instead of leaving the section empty.
- CallDiff is syntactic and TypeScript-only. Do not infer runtime behavior or claim
  coverage for dynamic calls that its output does not show.
- Generate this section fresh on every invocation, just like every other section.
...
### 8. Push and create or update the PR
...
current diff and CallDiff output (steps 1-7). Do NOT read the old PR body. Do NOT
carry forward any content from a previous description. Do NOT try to "merge" old
and new content.
